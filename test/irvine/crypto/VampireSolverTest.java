...
public class VampireSolverTest extends StandardIoTestCase {
  ...
  public void testCryptogramSetting() throws IOException {
    assertEquals("ABZ000A MV000", VampireSolver.cleanCryptogram(new BufferedReader(new StringReader("abZ019a mv.4$31\\? ")), false));
    assertEquals("ABZ000A MV\u0000000", VampireSolver.cleanCryptogram(new BufferedReader(new StringReader("abZ019a mv.4$31\\? ")), true));
  }
  ...
  public void testVampire() throws IOException {
    ...
    TestUtils.containsAll(s,
      ...
      "278.53193934606145 we live in a wonderful world that is full of beauty charm and adventure there is no end to the adventures that we",
